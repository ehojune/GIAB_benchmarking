...
| FASTQ 헤더로 시퀀싱 플랫폼 추정 (`detect_fastq_platform.sh`) | script | 리드 이름 규약만으로 판별 — Illumina 7필드 콜론, PacBio `m<무비명>/<zmw>/(ccs\|범위)`(Sequel/Revio 신형 + RS-II 구형 모두), ONT UUID·`runid=`, MGI `LxCxRx`. 헤더 불일치 시 리드 길이(<1000bp 숏리드/≥1000bp 롱리드)로 폴백. GIAB 특정 로직이 없어 다른 프로젝트에도 그대로 재사용 가능 |
