...
| 2 | **MGISEQ HG002 BAM 재생성** (정렬 BAM 33% 누락) | **재시도도 실패 — 결정적 버그 확정**. 156584가 23:36에 **bwa-mem2 Segmentation fault**(exit 139). 155525와 똑같이 **946,145,492 리드 처리 뒤** 같은 청크에서 죽었다(partial BAM 크기 0.003% 차) | 156584(실패) → 이분법 진단 잡(아래 이력) | `05_bwa_crash_bisect.sh`로 죽이는 리드 쌍·SIMD 빌드별 재현 확인 → 그 쌍만 빼거나 다른 빌드로 재정렬 → `VALIDATED.txt` → BAM 교체 → #3 | blind retry는 끝. 원본 파이프라인(09-21)도 같은 지점에서 끊겼으니 **#3(MGISEQ 세트 재개)을 그대로 돌리면 HG002는 또 잘린 BAM이 된다** |
