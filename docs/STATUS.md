...
2026-09-25 19:52–19:59 관리자 서버 확인. 완료 기준은 [SCOPE](SCOPE.md). 실행 중인 잡과 완료된 카탈로그 결과를 구분한다. somatic 누락 13행을 반영했고 TSV·Excel **442행 × 51열 대조 차이 0**이다.

| 작업 | 현재 상태 | 다음 한 번의 행동 |
|---|---|---|
| 업체 gd1~4 germline 재분석 | **156760–156763 모두 `r`**, 오류 기록 없음. 네 세트 모두 **30/265 rule**, 24샘플 Mark 로그가 19:54–19:56에 갱신됨. rule 수는 시간 기준 완료율이 아님 | 현재 run의 `__DONE__`·`error_list.txt`·BAM/VCF·간단 QC 확인 후 비교. 중복 재시작하지 않음 |
...
| GIAB 숏리드 미완료 기본 run | **155526–155529, 156690 모두 `r`**, CPU·Mark 처리량 증가. Illumina는 **38→72/118 rule**, HG002 Mark 완료. HG002·HG003 Manta는 signal 11; HG003 재시도 **156780도 exit 1** | [Manta 미해결 기록](runs/2026-09-25-illumina-hg003-manta-retry.md). 반복 재시도 없이 나머지 분석 유지. HG002 verifybamID는 **156781**로 별도 1회 실행. [QC 복구](runs/2026-09-25-illumina-hg002-verifybamid-retry.md). 20 set·53샘플은 미제출 |
