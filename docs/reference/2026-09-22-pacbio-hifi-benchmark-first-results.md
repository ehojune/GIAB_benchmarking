...
HG002 5런. `--pick ac --passonly -r 2000 -C 5000` + refine, refine 후 수치다.
truth `base cnt`는 전 런 28,123으로 같다.

> **아래 표는 `-t 22` 1차 실행(4런)의 값이다.** 같은 날 `-t 8`로 5런 전량 재실행해 전부
> 성공했고(exit 0, wall 247~257초), 그 수치로 갱신해야 한다 — `--collect` 결과를 받는 대로 교체한다.
> 스레드 수는 영역을 나눠 맡을 뿐이라 값이 바뀌면 안 되지만, poa 가 비결정적이라 보장은 없다.
...
**걸리는 건 recall이다.** 0.757~0.788 — pbsv가 truth 28,123개 중 **21,282~22,147개**를 찾고
**5,976~6,841개를 놓친다**(표의 TP-base / FN 열). precision은 0.90 언저리로 고르다.
즉 잘못 부르는 게 아니라 **못 찾는 것**이 문제다.
