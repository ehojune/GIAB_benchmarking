...
- manifest에 있는데 결과가 없음(미검사): **0** — 모든 키가 판정을 받았다. 체크섬까지 대조해 통과한 것은 PASS\* **77,511**이고,
  NOREF 3,752는 비교할 참조값이 없어 `md5sum`을 돌리지 않았으므로 검증 완료가 아니다. FAIL 39는 아래 후속 대조로 따로 닫았다.
