...
> **2026-09-23 후속: 교차시켰고, 2.18 은 부산물이었다.** 실제 차집합에 germline 같은 집합이 없다
> (Clair3 단독 0.87 · DV 단독 0.67, 공유 1.31). 다만 DV 가 진짜 변이를 버리는가 자체는 이것으로 못 가른다 —
> 아래 2026-09-23 항목.
...
## 2026-09-23 — "구간 밖 순차분 2.18" 은 부산물이었다. 질문 자체는 v5.0q smvar 로 닫는다

2026-09-22 에 "미확인" 으로 남긴 의심 — 두 caller 의 구간 밖 순 차이 ts/tv 2.18 이 germline 에
가까우니 DV 가 구간 밖에서 진짜 변이를 버린다 — 을 `63_caller_diff_regions.sh` 로 확인했다(잡 156585).

| 집합 | 구간 밖 SNV | 구간 밖 ts/tv |
|---|---:|---:|
| Clair3 만 | 389,628 | 0.87 |
| DV 만 | 302,632 | 0.67 |
| 둘 다 | 800,062 | 1.31 |

**의심의 근거가 사라졌다.** 2.18 은 두 단독 집합의 차(2.16)로 재현되고, 실제 차집합에는 germline 같은
집합이 없다. DV 단독이 더 tv 쪽이라 빼면 ts 가 남았을 뿐이다.

**질문은 안 닫혔다.** 단독 콜이 진짜인지 FP 인지는 구간 밖 진짜 변이의 ts/tv 를 알아야 판정되는데
그걸 잰 적이 없다(둘 다 부른 구간 밖 콜조차 1.31). 처음 판에서 "둘 다 FP 지배" 로 분류하고 "구간 밖은
교집합부터" 를 권고했는데, PR #39 Codex 리뷰가 "truth 없이 분류하지 말라" 고 짚었고 **동의해 둘 다
거뒀다.** 0.87/0.67 을 끝점(2.1, 0.5)에 대어 비중으로 푸는 것은 거부해 놓고, 말로 같은 추론을 한
것이었다 — 2026-09-21 ts/tv 사고와 같은 모양이다.

**닫는 방법:** `HG002_GRCh38_v5.0q_smvar`(T2T-Q100 유래)가 디스크에 있다. v4.2.1 이 빼놓은 어려운 영역
상당 부분까지 truth 가 있어 두 caller 의 FN·FP 를 구간 밖에서 직접 센다. 다음 작업으로 둔다.

**기본 caller(DV) 결정은 그대로다.** 구간 안 truth 로 검증한 것이고, 이번 결과는 그것을 뒤집지도
구간 밖으로 넓히지도 않는다.

## 2026-09-23 — HG002 dup 런은 V3.2.4 하나만 돌린다 (V2.3.4 는 오라벨로 취소)

`run_table` 의 dup_of 런 넷은 채점 **전**에 "같은 플로우셀 재베이스콜이라 중복 계산" 으로 빠졌다.
채점 후 헤드라인 "INDEL 은 베이스콜러가 지배한다" 의 근거 비교에 교란이 있다는 게 보였다:

| 런 | 베이스콜러 | 기기 | INDEL F1 |
|---|---|---|---:|
| HG002 UCSC | guppy 3.2.x | PromethION | .228 |
| HG002 guppy-V3.4.5 | guppy 3.4.5 | **MinION** | .556 |

.228 → .556 의 2.4배가 베이스콜러 때문인지 **기기·플로우셀** 때문인지 못 가른다.
**`guppy-V3.2.4` 는 V3.4.5 와 같은 MinION 플로우셀의 같은 run 들이다** — 돌리면 기기를 고정한 채
베이스콜러만 바뀐다. .23 근처면 베이스콜러가, .5 근처면 기기가 원인이다. Clair3 모델도 V3.4.5 와 같다.

**V2.3.4 는 취소했다.** 처음엔 같이 제출했는데(156588) run_table note 에 **"HG001 오라벨 플로우셀 3개가
섞여 있다 — 돌릴 거면 먼저 걸러야 한다"** 가 적혀 있었다. note 열을 안 읽고 제출했다. 파이프라인에는
read_id 거르는 단계가 없어 그대로 두면 HG001/HG002 혼합 샘플을 채점하게 된다. PR #39 Codex 리뷰가 짚었고
**시작 전에** 취소했다(qacct 기록 없음). 걸러서 다시 돌릴 가치는 낮다고 봤다 — Clair3 모델(g238)이 달라
베이스콜러만의 효과가 아니고, 걸러 내는 단계를 새로 만들어야 한다.

**나머지 릴리스가 깨끗한지 검증했다.** 오라벨 목록(GIAB `mis-labeled-sample_run-ids.txt`, run_id 26개)을
세 릴리스의 sequencing_summary run_id 와 대조했다(잡 156593, 2분):

| 릴리스 | run_id | 오라벨 |
|---|---:|---:|
| V2.3.4 (양성 대조) | 463 | **26** |
| V3.4.5 (이미 채점한 기본 런) | 437 | 0 |
| V3.2.4 (156587, 실행 중) | 437 | 0 |

양성 대조가 26/26 을 잡으므로 방법이 작동한다. **이미 발표한 V3.4.5 결과도 깨끗하다.** 두 릴리스가
437 로 같고 V2.3.4 의 463 에서 26 을 뺀 수와 정확히 같다(집합 일치까지는 대조하지 않았다).

**채점 자동화:** 계산 노드가 submit host 가 아니라(`qconf -ss` = 로그인 노드뿐) 서버 쪽
`-hold_jid` 사슬을 못 만든다. 로그인 노드에 30분마다 `qstat` 한 번 치는 감시 스크립트
(`$INFRA/jobs/dup_bench_poller.sh`)를 띄워 파이프라인이 끝나면 60·61 을 넣게 했다. 72시간 뒤 스스로
끝난다. V2.3.4 는 취소됐으므로 감시 스크립트가 60·61 을 넣어도 VCF 가 없어 SKIP 한다(무해).

**자원 충돌:** 156587 이 shepherd-1-8 을 30슬롯 잡고 있어 PacBio 의 층화 시험 잡(156590)이 그 뒤로
밀렸다(STATUS 5c 행에 PacBio 가 적었다). V2.3.4 를 취소해 추가로 밀지는 않는다.

## 2026-09-23 — v4.2.1 밖 성능은 v5.0q smvar 로 **한 채점 안에서 층화해** 센다 (빼지 않는다)

`63_caller_diff_regions.sh` 가 남긴 질문 — DV 가 v4.2.1 구간 밖에서 진짜 변이를 버리는가 — 은 구간 밖
truth 가 없어 ts/tv 로는 못 닫았다. HG002 에는 T2T-Q100 유래 `HG002_GRCh38_v5.0q_smvar` 가 디스크에 있다.

**쉬운 길을 버렸다.** v5.0q 로 채점한 전체 결과에서 v4.2.1 채점 결과를 빼면 "v4.2.1 밖" 이 나올 것 같지만,
두 truth 는 **겹치는 구간에서도 서로 다르다**(T2T 기반 재호출이라 같은 자리의 유전형·표현이 다를 수 있다).
차이에는 "v4.2.1 밖의 성능" 과 "두 truth 의 차이" 가 섞인다. 같은 날 순차분 2.18 로 한 번 속은 모양
그대로다.

**그래서 v5.0q 한 번의 채점 안에서 `hap.py --stratification` 으로 두 층을 센다.**
`64_v5q_strata.py` 가 v5.0q BED 를 v4.2.1 BED 와 대어 세 층으로 나눈다:

| 층 | 뜻 |
|---|---|
| `v5q_not_v421` | v4.2.1 이 덮는 염색체 안에서 v5.0q 에만 있는 구간 — **답**(v4.2.1 이 어렵다고 뺀 영역) |
| `v5q_and_v421` | 겹침 — 대조용. 여기 성능이 v4.2.1 채점과 크게 다르면 두 truth 차이가 크다는 신호 |
| `v5q_other_chrom` | v4.2.1 이 **아예 안 덮는** 염색체(chrX 등) |

마지막 층을 따로 뺀 것은 자체 리뷰에서 나왔다. v4.2.1 은 파일명대로 chr1~22 만 덮어서, 가르지 않으면 v5.0q 의
chrX 전체가 "v4.2.1 밖" 에 들어간다. 그건 어려워서 뺀 영역이 아니라 애초에 대상이 아니었던 염색체이고, HG002
는 남성이라 chrX 는 반접합이기도 하다. 한 층에 섞으면 "어려운 영역에서 caller 가 무엇을 버리나" 를 읽을 수
없다. 세 층의 합이 v5.0q 전체와 같지 않으면 스크립트가 죽고, 빈 층은 hap.py 에 넘기지 않는다.

`BENCH_STRAT_TSV` 를 v4.2.1 과 같이 주면 막는다 — 층화 결과가 기본 채점 디렉토리에 섞이고 FORCE=1 이면
기존 결과를 덮기 때문이다(이것도 자체 리뷰에서 나왔다).

**60_benchmark.sh 를 넓힌 방식.** PacBio 가 phase1 `STRAT=1` 에서 세운 규율을 따랐다 — 기본 실행(v4.2.1,
층화 없음)의 잡 스크립트는 이 기능 이전과 **바이트 단위로 같다**(heredoc 을 떼어 치환 비교로 확인). 버전별
결과·잡·jobid·임시 디렉토리는 `KEY`/`BENCH_SUB` 로 가르되 v4.2.1 은 기존 값 그대로다. 같은 경로를 쓰면
v4.2.1 결과를 덮고 중복 제출 가드와 hap.py 임시 디렉토리까지 공유한다. 층화 인자는 앞 공백을 포함해
`--logfile` 줄 끝에 붙인다.

**Codex 리뷰는 못 받았다** — 사용량 한도(2026-09-24 02:48 재개). GitHub 봇도 이 PR 에 반응이 없었다.
그래서 자체 리뷰로 대신했고 위 두 결함(chrX 혼입, v4.2.1 디렉토리 오염)이 거기서 나왔다. 독립 검증이 빠진
PR 이라는 사실을 남겨 둔다.

**확인한 것:** 구간 연산(교집합·차집합)을 염기 단위 무차별 계산과 무작위 500회 대조. `--collect-strata` 의
awk 는 ONT 공개 `extended.csv` 에 대어 Type × Subset 당 정확히 한 행(22행)이 나오는 것을 봤다 —
hap.py 는 Subtype(indel 길이)별로도 행을 내서 `Subtype=="*"` 를 안 걸면 한 구간이 여러 행이 된다.

## 2026-09-23 — [숏리드 세션] 03은 도는 병합의 concat.sh를 덮어쓰지 않는다

2차 적용 뒤 "set이 준비됐나"를 보려고 03을 다시 돌리는 일이 생길 텐데, 그때마다 03이 모든 병합 샘플의 concat.sh·list를 새로 썼다.
...
## 2026-09-24 — DV 는 v4.2.1 밖에서도 이긴다: 기본 caller 결정을 어려운 영역으로 넓힌다

9/22 에 "ONT R10 기본 caller = DeepVariant" 를 정하면서 **"벤치마크 구간 안에서만 검증된 결정"** 으로 한정해
뒀다. 9/23 에 구간 밖 순차분 2.18 로 "DV 가 구간 밖 진짜 변이를 버린다" 를 의심했고, 그 근거는 부산물로
판명됐지만 질문 자체는 truth 없이 못 닫았다.

v5.0q smvar(T2T-Q100) 로 층화 채점했다(잡 156640). v4.2.1 밖 상염색체 83 Mb 에서 **DV 가 recall·precision
둘 다 이긴다** — FN SNP −17.1% · INDEL −9.9%, FP −50.1% · −55.2%. 겹침 층 F1 이 v4.2.1 채점과 네 칸 다 0.001
안이라 두 truth 가 일관된다.

**정한 것:** 기본 caller 결정의 "구간 안에서만" 한정을 푼다. v4.2.1 밖 어려운 영역(v5.0q 가 덮는 곳)에서도
DV 가 낫다. 9/23 에 넣었던 "구간 밖은 교집합부터" 같은 권고는 이미 거뒀고 되살리지 않는다.

**성염색체는 결론에서 뺀다.** `v5q_other_chrom`(chrX·Y) 층은 F1 0.05~0.065 인데 FN ≈ FP ≈ (전체 − TP) 로
대칭이다. 변이를 놓친 게 아니라 **반접합 truth 와 이배체 호출의 유전형 불일치**로 보인다(HG002 는 남성).
`--squash-ploidy` 나 `FP.gt` 로 확인할 수 있지만 하지 않았다. 64 를 자체 리뷰할 때 이 층을 따로 뗀 것이
결론 오염을 막았다 — 섞었으면 "밖" 이 이 인공 수치에 끌려 내려갔을 것이다.

**9/23 의 ts/tv 방향을 truth 가 뒷받침하지 않았다.** 9/23 에 "Clair3 단독이 DV 단독보다 덜 FP 같다(ts/tv 0.87 >
0.67)" 를 방향으로만 적었는데, 이번에 v4.2.1 밖 83 Mb 에서 Clair3 의 FP 는 DV 의 두 배 이상이었다. **엄밀한
반례는 아니다** — 범위(63: v4.2.1 여집합 전체 / 이번: 83 Mb)와 집합(단독 콜 / 전체 FP)이 다르다. 그래서 "방향이
틀렸다" 가 아니라 **"방향이라도 ts/tv 를 FP 판정 근거로 쓰지 않는다"** 로 정한다. (첫 판은 "반대였다" 로 단정했다
— 자체 리뷰로 좁혔다. 같은 날 #39 에서 Codex 가 짚은 것과 같은 종류의 과장이다.)

## 2026-09-24 — [숏리드 세션] MGISEQ HG002 이분법 1차 결과: 651 페어 창, 모든 SIMD 빌드에서 죽음 → "창을 뺀 입력" 검증으로 간다

05 1차(156641, 20분): 죽은 청크(페어 473,072,746부터 333,582쌍)만으로 **재현됐고**(rc 139), **avx512bw·avx2·sse42 세 빌드 전부
죽었다** — SIMD 경로 버그가 아니라 빌드를 바꿔도 소용없다. 이분법은 9단계에서 **651 페어 창**(전체 [473,145,716, 473,146,367))까지
좁혔는데(그 크기에선 SIGABRT 134), 반으로 자르면 둘 다 안 죽었다. 죽는 곳이 worker_bwt(시드 찾기)라 그 청크의 삽입 크기 추정보다
앞이다 — 처음엔 `-I`로 분포를 고정하려 했으나 로그 순서를 다시 보고 버렸다(제출 전). 한 리드가 힙을 망가뜨리고 실제로 죽는지는 배치의
메모리 배치에 달린 것으로 본다(SIGSEGV와 SIGABRT가 섞이는 것도 그렇다).

정확한 한 리드를 못 찾아도 목적은 이룰 수 있다. bwa-mem2 `-K`는 배치를 **염기 수**로 자르므로 죽은 청크 앞의 배치는 입력에서 창을
빼도 원래 실행과 똑같고 이미 통과했다. 그래서 **죽은 청크 시작부터 끝까지를 창만 빼고** 같은 인자로 흘려 끝까지 통과하면(06, `EXCLUDE`),
창을 뺀 입력 전체가 bwa-mem2를 통과한다. 통과하면 파이프라인 입력에서 그 651쌍(7.1억 쌍 중 0.00009%)을 빼서 MGISEQ 세트를 재개한다.
원인 리드 찾기는 곁가지로 계속한다(05 2단계: 이상 리드 훑기, 블록 빼기, 후보가 적으면 한 쌍 빼기, 큰 청크에서 재확인).

## 2026-09-24 — [숏리드 세션] MGISEQ HG002: 창을 빼도 죽는다 → 레인 순서를 바꿔 검증한다

두 결과가 계획을 바꿨다. (1) 06(156644): 죽은 청크부터 끝까지를 **651 페어 창만 빼고** 흘렸더니 첫 배치에서 또 죽었다(rc 139,
처리 0). 창을 빼는 것은 해법이 아니다. (2) 05 2단계(156645): 창의 앞 528쌍 안 어느 33쌍 블록을 빼도 안 죽고, 뒤 123쌍 블록은 빼도
죽는다. 이상 리드(ACGTN 밖 문자·N 과다·긴 단일염기 반복·70bp 미만)는 0건. 특정 리드 하나가 아니라 **배치 안 리드 배치(순서·경계)에
달린 힙 오염**으로 본다 — 한 리드를 지목해 빼는 길은 접는다.

대신 **레인 순서를 바꾼다.** HG002 MGISEQ 병합 입력은 L03+L04 순서이고 죽는 청크는 L04 안(66% 지점)이다. L04+L03으로 이어 붙이면
L04의 배치 경계가 전부 옮겨진다(`-K`는 염기 수로 자른다). 리드는 서로 독립이라 정렬 뒤 결과에 순서는 상관없고, 리드를 하나도 버리지
않는다. fastp는 리드 단위로 결정적이므로(세 실행이 같은 리드 수에서 죽었다) 이미 있는 trimmed 파일을 L4→L3로 재배열해 bwa-mem2에
흘리면 재배열한 원본을 파이프라인 fastp에 넣었을 때와 같은 입력이 된다(`07_bwa_reorder_check.sh`, 약 6시간). 전체 1,428,410,152
리드가 끝까지 처리되면 파이프라인 입력을 L04+L03 순서로 다시 병합해 MGISEQ 세트를 재개한다. 새 bwa-mem2 빌드나 다른 정렬기는
파이프라인 일관성을 깨므로 마지막 수단으로 남긴다.

## 2026-09-24 — [숏리드 세션] MGISEQ HG002: 원인은 bwa-mem2 SMEM 버퍼 한도, 입력을 4구간 교차로 섞어 다시 만든다

07(156646, L04→L03 재배열)도 죽었는데 이번엔 단언문이 원인을 말했다: `num_smem: 1623546` /
`bwamem.cpp:970 mem_kernel1_core ... Assertion 'num_smem < *wsize_mem' failed` (rc 134, L04 안 약 2.2억 리드 지점 — 원래 죽던 곳
근처). bwa-mem2는 스레드 배치마다 SMEM 버퍼를 **배치의 총 염기 수**로 잡는데, 반복이 아주 심한 리드가 몰린 배치는 염기 수보다 SMEM이
많아진다. 단언이 켜진 경로에선 abort(134), 아니면 조용히 넘쳐 segfault(139) — 지금까지 본 증상이 다 이것으로 설명된다(한 리드로
안 좁혀지고, 몇 쌍만 빼거나 레인 순서만 바꿔도 같은 구역에서 또 죽는다). 최신 master도 같은 검사를 하고 넘치면
`Error [bug]: num_smem ... more than allocated space`로 멈춘다(재할당하지 않는다) — 새 빌드로 바꿔도 안 풀린다.

그래서 **그 구역을 배치들 사이로 흩는다.** 병합 입력(L03+L04)을 연속 4구간으로 나눠 한 레코드씩 돌아가며 내보내면(`interleave_shards.py`,
R1·R2 같은 순서, 리드 손실 0) 어느 배치에도 그 구역 리드가 1/4만 들어간다. 검증은 파이프라인과 같은 fastp → bwa-mem2(-Y -K 100000000)
→ primary 수 대조 → 정렬을 그 입력으로 끝까지 돌린다(`08_rebuild_sorted_bam.sh`, Codex 복구 스크립트의 인자 그대로). 통과하면 길이
둘: 검증된 정렬 BAM을 `tmp/04.sort`에 교체하거나, 파이프라인 입력을 이 순서로 바꿔 HG002를 처음부터 다시 돌린다(같은 도구·같은 인자라
결정적으로 통과). bwa 0.7.17(출력이 bwa-mem2와 같도록 설계됐고 SMEM을 동적으로 잡는다)은 서버에 없다 — 설치는 다운로드라 사용자 확인
없이 하지 않았다. 4구간으로도 죽으면 그때 이 길로 간다.

## 2026-09-24 — [숏리드 세션] Codex 리뷰(밀린 것) 반영 + 정정: "이상 리드 0건"은 검사가 돌지 않은 결과였다

한도가 풀린 뒤 로컬 Codex CLI(`adversarial-review`, 2026-09-23 밤 phase3 변경 전부)에 리뷰를 맡겼다. 판정 needs-attention, 8건.

**정정.** 05 2단계의 이상 리드 훑기는 awk `printf` 문자열 안에 줄바꿈 문자가 그대로 들어가 **문법 오류로 한 번도 돌지 않았고**,
빈 `anomalies.txt`를 "0건"으로 보고했다(위 2026-09-24 이분법 항목과 STATUS의 "이상 리드 0건"은 틀렸다). 고친 검사를 로그인 노드에서 651쌍
창에 돌리니 **150 bp 전체가 T인 리드**가 나왔고, 같은 청크의 대조 창 둘(청크 앞·뒤 각 651쌍)에는 반복 리드가 0이었다. poly-T 리드는
게놈 곳곳의 poly-A/T 구간과 맞아 SMEM을 대량으로 만든다 — SMEM 버퍼 초과 설명과 맞는다. fastp `--trim_poly_g`는 poly-T를 안 건드린다.

반영한 것(모두 테스트 추가): 03 — 새 샘플 dir을 만들기 직전 set을 다시 확인(파이프라인과 lock을 나눌 수 없어 틈은 샘플 하나 크기로
줄이는 데 그친다 — 파이프라인을 띄우는 동안 03을 돌리지 말 것), concat 파일을 쓸 때 병합과 같은 `.concat.lock`을 잡는다(확인만 하면
그 사이 시작한 병합이 R1은 옛 list, R2는 새 list로 이을 수 있다), sampleinfo는 `<xlsx>.lock`으로 읽기부터 교체까지 한 번에 하나만 +
임시 파일(.xlsx 확장자 유지 — openpyxl이 확장자로 형식을 가린다, 테스트에서 걸렸다)에 쓰고 `os.replace` + 백업 이름에 pid, `--lanes`는
살아 있는 잡을 먼저 전부 줄에 앉힌 뒤 제출. 생성기 — 모든 시트를 검증한 뒤에만 쓴다. 05 — 0은 통과, 134/139만 목표 충돌, 나머지(127·1·
137·143 등)는 판정 불가로 멈춘다. 06 — `EXPECTED_READS`를 줘야 PASS, 없으면 rc=0이어도 '미검증'(프로세스 치환의 압축 해제 실패는
pipefail에 안 잡힌다). 07은 처리 리드 수를 기대값과 대조하므로 그대로 뒀다. 테스트: 03 WSL 26/26, sampleinfo 3/3, 05·06·07·교차 가짜 bwa.

## 2026-09-24 — [PacBio 세션] QC ts/tv 하한 1.8 → 1.65, 종양 판정에 HG008T, 카탈로그는 입력 경로로 찾는다

- **ts/tv 하한.** DeepVariant 전장 PASS ts/tv 가 germline 19런에서 1.742~1.820 이라 하한 1.8 은 16/19 런을 걸었다.
  `62_tstv_regions.sh` 로 원인이 런이 아니라 **구간 밖 호출**임을 확인했다(구간 안 38건 전부 2.094~2.107).
  심도가 높을수록 더 낮다(정상 HG009N-WT-p4 84x 1.71). 1.65 는 그 최솟값 아래 — 크게 망가진 런만 잡는 선이다.
  phase2 는 이미 자기 코호트 실측으로 1.60 을 쓰고 있어 방향이 같다.
- **종양 판정.** `TUMOR_PAT` 이 `HG008-T`·`HG009T` 만 알아서 #8 에서 `HG008T-*` 로 등록한 NIST 9런이 정상 기준을
  받았다. `HG008T` 추가. phase2 판은 처음부터 `HG008T` 가 있다 — "35_review_qc.py 는 양쪽을 같이 고친다"
  규칙의 대상 로직이 이미 같아 phase2 는 안 고쳤다.
- **카탈로그 매칭.** `50_update_catalog.py` 의 key `(HG샘플, basename)` 이 HG008-T NIST 9행(시료 디렉토리마다 한 행)과
  run_table dataset(`HG008-T_bulk`/`_clones`)을 영영 못 맞췄다. 경고만 한 줄씩 나고 지나갔다. key 가 안 맞으면
  **입력 파일이 전부 그 행의 giab_path 아래인 실행 단위**를 찾는 두 번째 길을 넣었다. 로컬 dry-run 으로 확인:
  바뀐 것은 경고 9건 → 대기 9건과 새 2행뿐, 나머지 행의 판정은 그대로다.
- **카탈로그 category 수정(다운로드 세션 경로).** UMD Revio HG008-T p21·p41 두 행이 `other` 였다. 플랫폼 칸이
  "PacBio Revio HiFi" 이고 uBAM 이 있어 `pacbio_hifi` 가 맞다. 이 분류 때문에 PacBio 생성기·갱신기 어느 쪽도 못 봤다.
  매니페스트(`manifests/HG008/other.tsv`)는 옮기지 않고 생성기가 `mcat` 으로 골라 읽는다.

## 2026-09-24 — [숏리드 세션] MGISEQ HG002: 4구간 교차 입력으로 bwa-mem2가 끝까지 통과했다

156648(`08_rebuild_sorted_bam.sh`, 4구간 교차 입력): fastp before 1,451,463,284 → after 1,428,410,152 — 교차 전과 **똑같다**(리드를
하나도 잃지 않았고 fastp가 같은 리드를 걸렀다). bwa-mem2가 앞 세 실행이 죽던 946,145,492 리드를 지나 2,141 청크 1,428,410,152 리드를
끝까지 처리했고, primary 1,428,410,152 = fastp 출력. 원인(SMEM 버퍼 초과)과 해법(밀집 구역을 배치 사이로 흩기)이 맞았다.
교체는 (a) 정렬 BAM을 `tmp/04.sort`에 넣는 쪽으로 권한다(`09_swap_sorted_bam.sh`, 옛 BAM 보관, Snakemake lock 있으면 거부). 파이프라인
입력을 교차 순서로 바꾸는 (b)는 03의 병합 크기 검사와 어긋나 03 재실행 때 되돌아갈 수 있다. 교체와 set 재개는 파이프라인 상태를 바꾸는
일이라 사용자가 돌아와 정한다 — 정렬 끝(`VALIDATED.txt`)까지만 해 둔다.

## 2026-09-25 — 정정: 기본처리·QC·업체 비교로 GIAB 범위를 닫는다

사용자가 비용과 범위 확장을 줄이도록 지시했다. [SCOPE](SCOPE.md)를 정본으로 삼고 고정 카탈로그의 확보·필요 run·간단 QC는 계속 마무리한다. HG002/3/4 업체 비교가 본래 목표이며, 추가 자료의 `no-truth`는 새 somatic 개발 의무가 아니다. 기존 #9의 “QC만은 기각”과 “somatic 뒤 최종 보고” 조건을 철회한다. bioinfo 범용 개발은 별도 계속하되 GIAB 통합·추가 실험과 분리한다. GIAB 실행 담당은 한 명, 반복 리뷰·새 에이전트·자동 HARVEST는 보류한다. 원본·결과·서버 잡·작업 트리는 보존한다. 이전 STATUS와 backlog는 [reference](reference/README.md)에 보관했다.

## 2026-09-25 — 추가 승인: 기존 결과 정리와 사용자 주도의 somatic 전장

사용자가 PacBio 요약·ONT 완료 비교 초안·다운로드 집계 정리를 승인했다. 각 기존 담당이 한 번 정리하고 새 실험·반복 리뷰를 붙이지 않는다. somatic 전장도 진행하기로 했으나 사용자가 Claude Desktop PacBio 담당에게 직접 시작 지시한다. bioinfo 범용 코드와 협업하되 같은 코드·잡의 담당은 한 명으로 두고 HG002/3/4 기본 비교와 완료 조건을 분리한다.

## 2026-09-25 — gd2 원자료 지정과 조건부 입력 교정

사용자가 `data/jslink`를 gd2 원자료로 지정하고 입력 오류 시 재실행을 승인해 별도 교정 잡 156732를 제출했다. 이후 정본 이미지(1–3 KOR, 4–6 HG)를 제공하고 원래 `G000-gd2-20260819`를 정리해 입력·엑셀을 맞추도록 요청했다. 교정 잡만 중지하고 기존 결과는 보관한 뒤 원래 set을 준비해 사용자 검토 후 제출 경로로 인계한다. 사용자 표가 옛 추정 매핑보다 우선하며 동일 HG 시료와 동일 sequencing 입력을 구분한다.

07:30 실행 완료: 156732를 중지하고 옛 gd2를 보관한 뒤 원래 경로에 jslink 링크 12개를 구성했다. sampleinfo의 gd2 note·SEX 10칸을 백업 후 교정했으며 다른 셀·서식은 보존했다. 새 germline은 제출하지 않았다. gd1·3의 엑셀 순서와 gd4 입력 링크 충돌은 추가 확인사항으로 남겼다.

## 2026-09-25 — 정정: 4회사 전체 입력·엑셀 교정 후 germline 실제 제출

최신 사용자 지시가 앞선 gd2 준비·수동 제출 방침을 대체한다. 07:45:39 gd1~4를 정본 표(회사별 1–3 KOR-101/102/103, 4–6 HG002/3/4)에 맞춰 원래 SET_ID/outcome 아래 20자리 ID 24개·raw 링크 48개로 재구성했다. 엑셀 24행을 검증하고 gd1·3·4 30칸을 추가 교정했다(gd2는 앞 단계 완료); 성별은 실제 샘플 기준으로 유지했다. `/BiO/scratch/dyl/kbb/G000/backup/20260925-company-reset`에 옛 세트 6개·before.xlsx·계획을 모았고 내부 절대 링크 376개를 백업 대상으로 바꿔 유효성을 확인했다. 네 dry-run 통과 후 Java 17로 07:49:38 실제 제출: gd1 156760·gd2 156761 실행, gd3 156762·gd4 156763 대기. 분석 완료는 아니다. 옛 업체 결과·비교 캐시는 역사자료로만 보존하고, 교정 run 완료 후 이미 승인된 단순 truth 비교를 진행한다. 새 비교 잡은 제출하지 않았다. 실행 기록은 서버 `G000/.company-reset-20260925.json`; 기존 GIAB·somatic 잡은 유지한다.

### 2026-09-25 19:52 — 실패 단계만 한정 복구·완료 결과 반영

Manta HG003은 1스레드 재시도156780도 같은 signal 11이라 반복하지 않는다. HG002 Manta도 미완료로 남기고 기본9잡은 유지한다. HG002 verifybamID만 입력 검증 후 별도156781로1회 실행한다. 원출력·error_list는 보존한다. 카탈로그13행의 somatic 완료를 반영하되 T-BCM의 기존 GIAB Severus SV는 유지하고 자체 SNV/INDEL은 notes에 기록한다. 12개 me행은 전체13쌍 완료 수와 다르다.

[카탈로그·엑셀 대조 근거](reference/2026-09-25-somatic-catalog-sync.json): 다른429행 불변, 수식12개·링크16개·서식 보존.
...
## 2026-09-26 — 정정: 회사별 종료 후 일반 regermline부터 재시도

사용자가 각 gd의 germline 종료 후 regermline, 같은 단계 재실패 시 오류 중간 디렉토리를 비운 뒤 regermline을 지시했다. 앞선 253작업은 Codex가 `--forcerun`을 추가한 dry-run 결과라 일반 regermline의 동작으로 단정하지 않는다. 회사 QC gap을 영구 보류한다는 앞선 판단을 대체한다. 실패 로그·정상 결과·원자료는 보존하고 gd3는 현재 비교가 결과를 다 읽은 뒤 재개한다.

## 2026-09-26 19:53 — 일반 regermline 실제 동작 확인과 중지

사용자 지시대로 일반 regermline 156794/156795를 제출했으나 강제 옵션 없이도 각 253작업·fastp부터 재계산했다. 19:53:40 두 잡만 중지했다. 이전의 강제 옵션과 일반 재개를 구분한다는 기록은 유지하되, 실제 일반 실행도 같은 범위를 요구함이 확인됐다. 제거된 fastp 보고서 12샘플분은 Codex 책임으로 156796/156797에서 보고서만 복원한다. 원자료·CRAM/gVCF/SV/CNV는 변경 없음. [근거](runs/2026-09-26-gd23-regermline-and-DONE.md).

## 2026-09-26 20:15 — 실패 단계의 원래 명령만 개별 실행

사용자가 전체 재처리 대신 실패한 프로세스만 재실행하도록 요청했다. 완료 CRAM에서 임시 BAM을 복원해 같은 VerifyBamID·참조·설정으로 gd3 HG002 한 샘플부터 검증한다(156798). 성공 검증 전 정본 QC를 바꾸거나 나머지를 일괄 제출하지 않는다. 조립이나 재정렬이 아니라 형식 복원이며 기존 정렬·변이 결과를 보존한다. [기록](runs/2026-09-26-verify-only-pilot.md).

## 2026-09-27 — 검증된 QC 복구 확대와 비교 6건 진행

Verify 단독 pilot 156798이 정상 종료해 정본 QC에 반영했다. 같은 실패인 gd1 2건·gd2 3건·gd3 나머지 2건만 재시도한다. gd1·gd2 변이 파일·리드 수 검사 통과 후 기존 비교 6건을 병행하며 QC 재시도와 결과 해석을 구분한다. 전체 원자료 재처리는 하지 않는다. [잡·검증 기준](runs/2026-09-27-verify-recovery-and-company-comparison.md).

ONT 담당의 09-25 후속 비교를 확인해 09-24 문서의 베이스콜러 인과 표현을 정정했다. 원래 수치는 보존하고 후속 문서 링크를 붙였다. 새 분석이나 원인 연구는 하지 않았다. PR #68·#69는 PacBio 담당이 이미 병합했음을 GitHub에서 확인했으며, 이번 점검에서는 병합하지 않았다.
