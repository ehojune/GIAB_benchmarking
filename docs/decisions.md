...
**9/23 의 ts/tv 방향은 FP 방향과 반대였다.** 9/23 에 "Clair3 단독이 DV 단독보다 덜 FP 같다(ts/tv 0.87 > 0.67)"
를 방향으로만 적었는데, truth 로는 Clair3 의 v4.2.1 밖 FP 가 DV 의 두 배 이상이다. ts/tv 는 FP 의 **양**을 재지
않는다 — 그날 "크기는 못 잰다" 로 한정한 것이 맞았고, 방향도 근거가 못 됐다. (두 분석의 "밖" 범위는 다르다:
63 은 v4.2.1 여집합 전체, 이번은 v4.2.1 염색체 안 83 Mb.)
