...
`64_v5q_strata.py` 가 v5.0q BED 를 v4.2.1 BED 와 대어 **겹침**(`v5q_and_v421`)과 **v5.0q 에만**
(`v5q_not_v421`) 으로 나눈다. 뒤엣것이 답이고, 앞엣것은 대조용이다 — 겹침 층의 성능이 v4.2.1 채점과 크게
다르면 두 truth 의 차이가 크다는 신호라 v5.0q 에만 층의 해석도 조심해야 한다. 두 층의 합이 v5.0q 전체와
같지 않으면 스크립트가 죽는다.
