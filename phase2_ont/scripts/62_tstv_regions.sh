...
#
#   bash phase2_ont/scripts/62_tstv_regions.sh                 # truth 있는 런 전부
#   bash phase2_ont/scripts/62_tstv_regions.sh HG005.UCSC_Ultralong_OxfordNanopore_Promethion
#   TSV=out.tsv bash phase2_ont/scripts/62_tstv_regions.sh     # 표를 파일로도
#
# **왜 이게 필요한가.** 2026-08-27 에 "전 런 ts/tv 가 germline 기대치 2.0~2.1 보다 낮다"를
# 열린 질문으로 남겼다. 2026-09-18 에 hap.py 로 "구간 안에서는 FP 가 거의 없다"를 확인하고
# 구간 밖 값을 **빼기로 추정**했다 (R9/HG005 1.33). 그 추정은 "구간 안 ts/tv = 2.1" 이라는
# 가정에 기대고, 가정을 2.0 으로 바꾸면 답이 눈에 띄게 움직인다. 이 스크립트는 구간 안을
# 가정하지 않고 **직접 센다.**
#
...
rows=()
run_one() {  # dsid sample dataset dv_model
    local dsid=$1 sample=$2 dataset=$3 dv=$4 bed id base c vcf
    ...
    local callers="clair3"
    [ "$dv" = - ] || callers="clair3 deepvariant"
    for c in $callers; do
