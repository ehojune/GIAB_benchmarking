...
def load_ext():
    """ext_manifest.tsv -> (리드, 인덱스). 둘 다 {dsid: {relpath: bytes}}.

    리드만 샘플시트 행이 되지만 **인덱스도 준비 상태 판정 대상이다** — aligned_bam 진입은 .bai를
    명시적으로 넘기므로 BAM만 있고 .bai가 없으면 파이프라인이 거부한다. inputs_manifest에 넣어
    두지 않으면 10_submit이 그 상태를 ready로 보고 잡을 넣는다 (KIND=reads 로만 받았을 때 실제로
    생기는 상황이다). evidence 행(sequencing summary)은 파이프라인 입력이 아니라 둘 다 아니다.
    """
    reads, index = {}, {}
    with open(PHASE2 / "ext_manifest.tsv", newline="", encoding="utf-8") as fh:
        hdr = fh.readline().rstrip().split(chr(9))
        for line in fh:
            if not line.strip() or line.startswith("#"):
                continue
            r = dict(zip(hdr, line.rstrip().split(chr(9))))
            tgt = {"reads": reads, "index": index}.get(r["kind"])
            if tgt is None:
                continue
            tgt.setdefault(r["dsid"], {})[r["relpath"]] = int(r["bytes"])
    return reads, index
...
def main():
    ...
    for run in RUNS:
        ...
        # 샘플시트가 인덱스를 **실제로 가리킬 때만** 준비 상태에 넣는다 (load_ext 주석 참조).
        # aligned_bam_realign 은 정렬을 버리고 다시 하므로 .bai 를 안 쓴다 — 그런데도 요구하면,
        # 문서화된 KIND=reads 로 받았을 때(인덱스를 건너뛴다) 입력이 다 있는데도 미완으로 보여
        # 10_submit 이 제출을 거부한다.
        if run["entry"] == "aligned_bam":
            for ip in sorted(ext_index.get(dsid, {})):
                input_rows.append((dsid, ip, ext_index[dsid][ip]))
