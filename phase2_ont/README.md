...
**ts/tv 열린 질문은 닫혔다** (2026-09-22, [`62_tstv_regions.sh`](scripts/62_tstv_regions.sh) 직접 측정).
전 런의 genome-wide ts/tv 가 germline 기대치 2.0~2.1보다 낮은 것(R9 1.86~1.97, R10 1.65~1.77)은
**벤치마크 구간 밖 SNP 때문이다.** benchmark BED로 갈라 세면:

| | 구간 안 | 구간 밖 |
|---|---|---|
| ts/tv (10건 전부) | **2.0972~2.1400** | **1.0916~1.4839** |

구간 안은 germline 기대치와 같고 구간 밖만 낮다. **콜러나 파이프라인 문제가 아니다.**

**ts/tv 는 SNP 지표이고 FP 판정 근거가 아니다.** 치환 구성을 잴 뿐 FP 가 얼마인지는 재지 않는다. 구간 밖
진위는 구간 밖 truth 로 센다 — v5.0q 로 세면 R10 SNP precision 이 DV 0.943 · Clair3 0.890 이었다
([reference](../docs/reference/2026-09-24-ont-r10-v5q-stratified.md)). 구간 밖 ts/tv 는 베이스콜러가 좋을수록
낮고(R10 1.15 < guppy 4.2.2 1.30 < 3.2.x 1.48) R10 이 구간 밖 SNP 를 더 많이 부른다(1.20M vs 0.97~1.12M).
그 콜의 진위는 이 숫자로 가리지 않는다.

(계산은 ts·tv **카운트**로 한다. 비를 전체 SNV 수로 가중하면 1.15가 나오는데 틀린 값이다 —
2026-09-21 정정. 2026-09-18의 추정치 1.33은 이번 실측 1.3064로 확인됐다.)
...
**단 R9 안의 계단은 베이스콜러만의 효과가 아니다** (2026-09-25). 같은 MinION 플로우셀을 guppy 3.2.4 로 베이스콜하면
INDEL F1 이 .623 으로 3.4.5(.556)보다 높다. UCSC(.228, 역시 guppy 3.2.x)와의 격차는 버전으로 설명되지 않는다.
나머지 원인(기기·플로우셀·라이브러리)은 가르지 않았다 — [상세](../docs/reference/2026-09-25-ont-guppy324-vs-345-same-flowcell.md).
