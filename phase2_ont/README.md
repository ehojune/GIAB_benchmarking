...
ts/tv ≈ 1.33이어야 한다 — FP가 섞인 값으로 자연스럽다. 콜러나 파이프라인 문제가 아니다.
(가중치를 그 비의 분모인 Tv로 잡아야 한다. 전체 SNV 수로 가중하면 1.15가 나오는데 틀린 값이다.
가장 헷갈리지 않는 길은 `Ti = N·r/(1+r)`로 카운트를 복원해 빼는 것 — 2026-09-21 정정.)
