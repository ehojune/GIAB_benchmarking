...
def main():
    ...
    for r in RUNS:
        ...
        if r["existing"]:   # make_samplesheets.py 가 만든 시트
            ...
            pending.append((sheet, sheet_rows))   # 쓰기는 전부 검증된 뒤 한꺼번에 — 오류가 나면 기존 시트를 반쪽짜리로 덮지 않는다(Codex 리뷰)
        ...
        out_rows.append(dict(dsid=r["dsid"], set=r["set"], label=r["label"], hg=r["hg"], sex=SEX[r["hg"]], tier=r["tier"],
                             ...
                             note=r["note"]))
    if errors:
        print("ERROR: 아무 파일도 쓰지 않았다:", *errors, sep="\n  "); sys.exit(1)
    for sheet, sheet_rows in pending:
